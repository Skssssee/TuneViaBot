...
class YouTubeAPI:
    ...
    # -------------------------------------------------
    # MUST RETURN TRUE (BOT FLOW)
    # -------------------------------------------------
    async def exists(self, link: str, videoid=None) -> bool:
        return True

    # -------------------------------------------------
    # EXTRACT URL FROM MESSAGE (REQUIRED)
    # -------------------------------------------------
    ...
    async def track(
        ...
        details = {
            ...
            "link": link,
            "vidid": vidid,
            "duration_min": "0:00",   # 🔥 SAFE FOR time_to_seconds
